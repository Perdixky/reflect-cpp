...
and Clang 20+, GCC 14+, or MSVC 19.34+ (toolset 14.34+). AppleClang is not
supported. The module interface units live in the [`modules/`](modules)
directory and are enabled per format via the same `REFLECTCPP_*` CMake options
as the headers. `REFLECTCPP_MODULES` controls module support: `AUTO` (the
default) enables modules on supported compilers, `ON` requires a supported
compiler, and `OFF` provides only the header-based interface.
